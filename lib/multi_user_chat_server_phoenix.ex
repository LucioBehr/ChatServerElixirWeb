defmodule MultiUserChatServerPhoenix do
  ...
  def start_link(_), do: GenServer.start_link(__MODULE__, %{groups: %{}, users: %{}, priv_messages: %{}, group_messages: %{}, id_msg: 0, id_group: 0, id_user: 0}, name: __MODULE__)
  ...
  def enter_group(user_id, group_id) do
    GenServer.call(__MODULE__, {:enter_group, user_id, group_id})
  end
  ...
  def handle_call({:create_user, user_name}, _from, state) do
PrivLogic.create_user(user_name, state)
end
  ...
  def handle_call({:enter_group, user_id, group_id}, _from, state), do: GroupLogic.enter_group(user_id, group_id, state)
