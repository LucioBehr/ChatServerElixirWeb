defmodule MultiUserChatServerPhoenixWeb.Groups.Logic do
  def create_group(
        user_id,
        group_name,
        %{groups: groups, group_messages: group_messages, users: users} = state
      ) do
    new_group = %{
    ...
    {:reply, "group #{state.id_group + 1} created",
     %{
       state
       | users: updated_user,
         groups: create_group,
         id_group: state.id_group + 1,
         group_messages: Map.put_new(group_messages, state.id_group + 1, %{})
     }}
  end
  ...
  def send_group_message(
        ...
        group_id,
        content,
        %{group_messages: group_messages, groups: groups, users: users, id_msg: id_msg} = state
      ) do
    ...
    case user_id in Map.get(state, :groups, %{})[group_id][:users] do
      true ->
        ...
        updated_sender =
          %{sender | sended_messages: sender.sended_messages ++ [new_id_msg]}

        messages = Map.get(group_messages, group_id, %{})

        updated_messages =
          Map.put(messages, new_id_msg, %{user_id: user_id, content: content})

        new_state =
          %{
            state
            | groups: Map.put(groups, group_id, updated_group),
              id_msg: new_id_msg,
              users: Map.put(users, user_id, updated_sender),
              group_messages: Map.put(group_messages, group_id, updated_messages)
          }
    ...
    case user_id in Map.get(state, :groups, %{})[group_id][:users] do
      ...
      false ->
        ...
        new_state = %{
          state
          | groups: Map.put(groups, group_id, updated_group),
            users: Map.put(users, user_id, updated_user)
        }
