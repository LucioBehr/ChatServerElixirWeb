defmodule MultiUserChatServerPhoenixWeb.PrivateController do
  ...
  def alter_name(conn, params) do
    response =
      Operations.alter_name(params)
      |> validate_response(fn _ ->
        "User #{params["user_id"]} renamed to #{params["new_user_name"]}"
      end)
  ...
  def get_sended_private_messages(conn, params) do
    response =
      Operations.get_sended_private_messages(params)
      |> validate_response()
  ...
  def get_received_private_messages(conn, params) do
    response =
      Operations.get_received_private_messages(params)
      |> validate_response()
  ...
  def get_pending_messages(conn, params) do
    response =
      Operations.get_pending_messages(params)
      |> validate_response()
  ...
  def send_priv_message(conn, params) do
    response =
      Operations.send_priv_message(params)
      |> validate_response(fn _ ->
        "message #{params["content"]} sended from user #{params["sender"]} to user #{params["receiver"]}"
      end)
